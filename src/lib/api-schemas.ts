...
export const sendMessageSchema = z.object({
  ...
  previousMessages: z.array(messageSchema).max(12).optional().default([]),
...
export const scoreDebateSchema = z.object({
  debateId: debateIdSchema,
  topic: topicSchema,
  messages: z.array(messageSchema).min(2, 'At least 2 messages required for scoring').max(12),
...
export const judgeDebateSchema = z.object({
  debateId: debateIdSchema,
  topic: topicSchema,
  messages: z.array(messageSchema).min(2, 'At least 2 messages required for judging').max(12),
});
